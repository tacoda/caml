---
description: Commit and pull request rules for caml
---
...
- Bump `s.version` in `caml.gemspec` for releases.
- `Gemfile.lock` currently lags the gemspec (locks `caml 0.1.0` while the gemspec is `0.1.1`). Run `bundle install` whenever the gem version changes so the lockfile and gemspec agree before commit.
